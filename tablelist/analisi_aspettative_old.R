...
usernames <- c("Carlo","Claudio","Gabriele","Reto","Riccardo")
...
nb.pre.index=4  ## variable used to determine the number of sub indices; the first three column are 
                ## of the data frame containing the data are Country, Sector, Ticker, Ticker_AAA i.e. nb.pre.index = 3 
...
## utility function (to be added to the utilities package
...
{
  dati <- get.table(odbcConnection, tableName="#Lista_parziale_con_Null_da_importare",fieldNames=c("Ticker","Ticker_AAA","Paese"))
    ...
    if (nbNA==0)
      {
        ...
        ord <- c("Total obs.","Min.","1st Qu.","Median","Mean","3rd Qu.","Max.","Stdv","Range")
      }
    else
      {
...
writeSummaryStatistics <- function(x)
  {
    ...
    if (x$statistics["Total obs."]!=x$statistics["Valid obs."])
      {
...
start_window <- function()
{
      ...
      if (length(df.index)>0)
        {
          ...
          cb.index[["reset"]]()
          #cb.index[["set.selection"]](df.index[1])  
        }
      else
        {
  ...
  onOnlyAaaUpdate <- function()
    {
      updateNotebook(updateFilter=FALSE)
    }

  onShowHistogram <- function()
    {
      if(exists("dataSelection")) updateHistogram(dataSelection[["df.data"]])   
    }

  onShowBoxplot <- function()
    {
      if(exists("dataSelection")) updateBoxplot(dataSelection[["df.data"]])   
    }
  ...
  get.data <- function(Table,index,datumSql,onlyAaaTicker,country,sector,useFactors=FALSE)
    {
      ...
      nbDataFields <- length(df.fieldNames[desiredFieldNames,"Nome_campo"])
      ...
      if (useFactors)
        {
          df.data <- get.table(odbcCon,query=query,as.is=FALSE)   
        }
      else
        {
          ## change the number of as.is if you modify the number of fields!
          df.data <- get.table(odbcCon,query=query,as.is=1:4)        
        }
        ...
        { 
          ...
          for (q in 1:nb.pages) 
            {        
              ...
              envir[[tabNameCounter]][["nb.countryObs"]] <- nb.countryObs
              envir[[tabNameCounter]][["nb.obs"]] <- nb.obs             
                  ...
                  for (country in levels(countries)[inizio:fine]) 
                    {
                      ...
                      hist(subIndexData[(desiredCountry & validRows),2],freq=TRUE,breaks=11,main=country,
                      xlab=paste(nb.countryObs[country]," observations",sep=""))
                    }
  ...
  updateNotebook <- function(updateFilter=TRUE)
    {
    ...
    # get the data
    dataSelection <- get.data(Table,index,datumSql,onlyAaaTicker,country,sector)
    
    ## remove the tablelist
    if (exists("tbl.data")) tkdestroy(tbl.data[["frame"]])
    nbColumns <- dim(dataSelection[["df.data"]])[[2]]

    ## create a new tablelist with the new data
    tbl.data <<- create_tablelist(parent=tbn.notebook[["frame1"]],dataFrame=dataSelection[["df.data"]][-4],width=130,
                                 height=42,withScrollBarX=TRUE,withScrollBarY=TRUE,colFormats=
                                 c(rep("no",3),rep("round2",nbColumns-3))
                                )
    ...
    if (updateFilter)
      {    
        query = paste("SELECT DISTINCT A.Paese, B.BrancheMarkowitz FROM ",Table,"_storico A ",
                      "LEFT OUTER JOIN Lista_da_importare_storico B ON A.Ticker = B.Ticker AND A.Id_Provider = B.Id_Provider ",
                      "INNER JOIN Lista_da_importare_storico C ON A.Ticker = C.Ticker WHERE A.Data_download = '",
                      datumSql,"' AND B.BrancheMarkowitz IS NOT NULL ",
                      " ORDER BY B.BrancheMarkowitz",
                     sep=""
                     )

        sectorNewValues <- get.table(odbcCon,query=query)
        if (dim(sectorNewValues)[[1]]>0)
          { 
            cb.sector[["modify.values"]](c("",unique(sectorNewValues[,"BrancheMarkowitz"])))
            tmp <- unique(sectorNewValues[,"Paese"])
            tmp <- tmp[order(tmp)]
    ...
    ## get the data                                                     
    dataSelection <<- get.data(Table,index,datumSql,onlyAaaTicker,country,sector,useFactors=TRUE)
    fieldnames <- dimnames(dataSelection[["df.data"]])[[2]][-(1:4)]
## assign("a",dataSelection,envir=globalenv())
   
    updateHistogram(dataSelection[["df.data"]])
    updateBoxplot(dataSelection[["df.data"]])
      ...
      if (reportAllRequired)
        {
          ...
          X <- dataSelection[["df.data"]][,fieldnames]
          if (is.vector(X))
            {
              X <- data.frame(X)
              colnames(X) <- fieldnames
            } 
      ...
      if (reportByCountryRequired)
        {
          ...
          for (field in fieldnames)
            {
              tmp <- list()
              countryReport[[field]] <- tapply(X=dataSelection[["df.data"]][,field],
                     INDEX=dataSelection[["df.data"]][,"Country"],FUN=summaryStatistics)
      ...
      if (reportBySectorRequired)
        { 
          ...
          for (field in fieldnames)
            {
              tmp <- list()
              sectorReport[[field]] <- tapply(X=dataSelection[["df.data"]][,field],
                     INDEX=dataSelection[["df.data"]][,"Sector"],FUN=summaryStatistics)
      ...
      if (reportByCountrySectorRequired)
        {
          ...
          for (field in fieldnames)
            {
              tmp <- list()
              countrySectorReport[[field]] <- tapply(X=dataSelection[["df.data"]][,field],
                     INDEX=dataSelection[["df.data"]][,c("Country","Sector")],FUN=summaryStatistics)
...
#assign("countryReport",countryReport,envir=globalenv())
  ...
  cb.provider <- create_combo(parent=lf.selection[["labelFrame"]],values="UBS",
                 startValue="UBS",editable=FALSE,modifyCommand=pippo)
  ...
  df.dates <- get.table(odbcCon, query=query)
  cb.dates <- create_combo(parent=lf.selection[["labelFrame"]],values="",
  ...
  cb.index <- create_combo(parent=lf.selection[["labelFrame"]],values="",
  ...
  cb.period <- create_combo(parent=lf.selection[["labelFrame"]],values="",
  ...
  cb.country <- create_combo(parent=lf.selection[["labelFrame"]],values="",
                           editable=FALSE,modifyCommand=onCountryUpdate)
                            
    ## Sector
  l.sector <- create_label(parent=lf.selection[["labelFrame"]],value="Sector")
  cb.sector <- create_combo(parent=lf.selection[["labelFrame"]],values="",
