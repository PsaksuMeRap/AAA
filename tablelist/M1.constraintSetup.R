  ...
  else
    {
      ...
      if (answer == "yes")
      ...
      else
        {
          ...
          nb.multiple.restr = select.result(channel=odbcCon,query=query)

          if (is.na(nb.multiple.restr)) nb.multiple.restr <- 0
...
create.constraintsWindow <- function()
{
  ...
  ## update the constraints tables
  constraintSetup()

  ## create the function to update the colors of the multiple tablelist widget
  adjustColor <- function(tablelist,byColumn=1,colors=c("linen","#e0e8f0"))
    {
      ## get the tablelist column of Id
      Id <- tbl.multipleConstr[["get.columns"]](byColumn)
      nbRows <- dim(Id)[[1]] 
      if (nbRows == 0) return
      ...
      for (index in indices)
        {
          toChange = tmp[Id[,1] == index]  
          tablelist[["changeRowColor"]](index=toChange,color=color)
          if (color==colors[1]) color=colors[2] else color=colors[1]   
        }
    }
  ...
  onNew <- function()
    {
      ...
      onInsert <- function()
        {
          ...
          ## adjust the color
          adjustColor(tablelist=tbl.multipleConstr)
      ...
      ## valuesSqlType <- c("nvarchar","nvarchar","bit")   not required any more
      ...
      values <- tcl.get.table(odbcConnection=odbcCon,tableName="B_monete_e_settori_selezionati",query=query,
                              ...
                              updateDb=F
                              # ,odbcFieldTypes=valuesSqlType not required any more
                              )
  ...
  onRemove <- function()
    {
      ...
      result <- sqlCommand(channel=odbcCon,query=query)
      query <- paste("UPDATE B_Restrizioni_multiple SET NumeroRestrizione = NumeroRestrizione - 1 WHERE NumeroRestrizione >",constraintId)
      result <- sqlCommand(channel=odbcCon,query=query)
  ...
  ## end functions used by the multiple constraints buttons
  ...
  ## 1.1) get the data
  ## RiskFreeSqlType <- c("nvarchar","nvarchar","float","float")  not required any more
  ...
  riskFreeConstr <- tcl.get.table(odbcConnection=odbcCon,
                                  ...
                                  ## ,odbcFieldTypes=RiskFreeSqlType not required any more
                                  )
  ...
  ## 2.1) get the data
  ## currencySqlType <- c("nvarchar","float","float")   not required any more
  ...
  currencyConstr <- tcl.get.table(odbcConnection=odbcCon,
                                  ...
                                  ## ,odbcFieldTypes=currencySqlType  not required any more
                                  )
  ...
  ## 3.1) get the data
  sectorSqlType <- list(Branche="nvarchar",Min="float",Max="float")
  ...
  ## 4.1) get the data
  currencySectorSqlType <- list(Moneta="nvarchar",Branche="nvarchar",Min="float",Max="float")
  ## names(currencySectorSqlType) <- c("Moneta","Branche","Min","Max")
  query <- "ORDER BY Moneta,Branche"
  ...
  tbl.currencySectorConstr <- create_odbcTablelist(parent=lf.currencySector[["labelFrame"]],data=currencySectorConstr,withScrollBarX=TRUE,
                                                   alignCols=c("center","center","right","right"),withScrollBarY=TRUE,width=45,
                                                   height=40,editable=list(Min="Entry",Max="Entry"),
                                                   colFormats=c("no","no","percent2","percent2"),updateDb=T)

  ## 5) <multiple> constraints
  ## 5.1) get the data
  multipleConstraintsSqlType <- c(NumeroRestrizione="int",Moneta="nvarchar",Branche="nvarchar",
                                      TipoRestrizione="nvarchar",ValoreRestrizione="float")
  ...
  onSave <- function()
    {
      tbl.currencySectorConstr[["finishediting"]]()
  ...
  onCancel <- function() {tkdestroy(window.constraints);odbcClose(odbcCon);start_window(updateSelection=TRUE)}
  ...
  tkgrid(tbl.currencySectorConstr[["frame"]],padx=padx,pady=c(pady,45))
  tkgrid(lf.currencySector[["labelFrame"]],padx=padx,pady=pady,column=1,row=1,rowspan=3,sticky="ns")

  tkgrid(tbl.multipleConstr[["frame"]],padx=padx,pady=pady)
