...
setwd("/home/claudio/Dropbox/analisi serie storiche/capitalgest")
...
filterFunds <- function(strategia="CTA",tsFreq="Monthly",moneteDaUtilizzare= 
             c("usd","eur","gbp","chf","jpy"),
             dateForCrossValidation=as.Date("2007-07-31",format="%Y-%m-%d")) {
  ...
  dati <- importaHedgeFunds(strategia,soloValute=moneteDaUtilizzare)
  prezzi_matrix <- dati$matrix
  AUM.df <- dati$AUM.df
  rm(dati)


  # ---------------------------------------------------------------------------
  # --------------------- FILTRO CROSS VALIDATION -----------------------------
  # ---------------------------------------------------------------------------

  # Applica un altro filtro per la cross validation 
  selection <- as.Date(rownames(prezzi_matrix)) <= dateForCrossValidation
  prezzi_submatrix <- CTA_matrix[selection,]
  rm(selection)


  # ---------------------------------------------------------------------------
  # --- seleziona i fondi della strategia corrispondente ----------------------
  # ---------------------------------------------------------------------------

  prezzi.df <- as.data.frame(prezzi_submatrix)
  dates <- as.Date(rownames(prezzi.df),format="%Y-%m-%d")
  dataIniziale <- min(dates)
  dataFinale <- max(dates)
...
ELS <- importaHedgeFunds("ELS",soloValute=moneteDaUtilizzare)
...
MAC <- importaHedgeFunds("Macro",soloValute=moneteDaUtilizzare)
