...
delete.table <- function (table,channel) {
	sqlClear(channel,sqtable=table,errors=TRUE)
}
...
#                 )
	if (numericTicker) {
		...
		query <- paste("CREATE TABLE ", table, " (Ticker1 varchar(20) NOT NULL, ",
				"Ticker2 varchar(20) NOT NULL, Covarianza float ",
				"CONSTRAINT key_", table, " PRIMARY KEY (Ticker1,Ticker2))", sep = ""
		)		
	}
	result <- sqlCommand(channel,query,errorText)
...
put.cov <- function (Sigma, table="Covariances", channel=channel, delete=FALSE,
		numericTicker=FALSE, drop = FALSE) {
	
	if (drop) {
		if (tableExist(myConnection=channel,tableName=table)) {
	...
	sqlSave(channel, df.data, tablename = table, rownames=FALSE, fast=TRUE, append = TRUE, safer = TRUE)
}
