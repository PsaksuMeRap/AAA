...
pseudoTrueValues <- function(ma,p=1) {
	...
	## ma[2] is always 1.0, the weight of lag0
	...
	if (p==0) return(list(pseudoTrueValues=ma[1],b=Alpha))
	...
	return(list(pseudoTrueValues=c(pseudoCostante,-x),b=t(tildeA)%*%tildex))
