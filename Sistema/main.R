...
fundPortfolios <- filterS4List(dati,"Cliente",value=fondi)
...
if (FALSE) {
	...
	bacepTestSuite <- testSuiteFactory(testSuiteName="Fondi OpenCapital",directories="./FondiNew")

	results <- lapply(bacepTestSuite@testSuitesParsed,applyTestSuite,fundPortfolios)
}
