...
baseDirectory = "M:/workspace/AAA/ottimizzatore"
setwd(baseDirectory)

# load("./unitTests/data/excel_input.RData")
load("\\\\usi\\dfs\\utenti\\O\\ortellic\\Desktop\\excel_input.RData")
...
if (Sys.info()["sysname"]=="Linux") {
	...
	rLibrary = "algencan.so"
}
if (Sys.info()["sysname"]=="Windows") {
	if (Sys.info()["machine"]=="x86"){
		rSolverFolder ="/solver-32/"
	}
	rLibrary = "algencan.so.dll"
}
...
if (any(ottimizSenzaBench$feasible))
{
	...
	# RHome <- R.home()
	dyn.load(paste(rSourceDirectory,rSolverFolder,rLibrary,sep=""))

	
	# setwd(paste(baseDirectory,rOutputdirectory,sep=""))	
	setwd(paste("M:/workspace/AAA/ottimizzatore/",rOutputdirectory,sep=""))
