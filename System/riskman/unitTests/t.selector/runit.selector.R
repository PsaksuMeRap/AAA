...
test.shouldExtractPositionsFromSelectionString <- function() {
...
test.shouldExtractPositionsFromSelectionCriteriaList <- function() {
	...
	# test1
	selectionString <- new("SelectionString","security:Equity,Bond & currency!:GBP + security:Index_certificate & amount:>=294333.50USD")
	factorStrings.l <- split(selectionString)
	selectionCriteriaList <- new("SelectionCriteriaList",lapply(factorStrings.l,toSelectionCriteria))
	
	should <- new("Positions",list(p1,p2,p4,p6))
	
	result <- selector(selectionCriteriaList,positions)
	checkEquals(result,should)
	
	# test2: with an empty positions
	should <- new("Positions")
	result <- selector(selectionCriteriaList,new("Positions"))
	checkEquals(result,should)
