...
create_position <- function() {
	...
	position$create <- function(name=NA_character_,currency="CHF",
			amount=0.0,origin=NA,instrumentType=NA_character_) {
		position$name <<- name
...
create_repositoryEquities <- function() {
	repository <- list()
...
create_investmentPolicyAttributes <- function() {
	...
	# Lo strumento è trattato in una borsa valori
	attributes[["a1.a"]] <- FALSE
	
	# La borsa è riconosciuta valida 
	attributes[["a1.b"]] <- list(exchangeName=NA_character_,isValid=FALSE)
	
	# E' nuova emisione?
	attributes[["a3.a"]] <- FALSE
	
	# Se a3.a è vera, soddisfa la condizione a.3?
